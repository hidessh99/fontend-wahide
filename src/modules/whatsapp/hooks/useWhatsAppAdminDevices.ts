...
import { AdminDeviceItem } from "../types/admin.types";
import { whatsappAdminApi } from "../api/admin.api";
import { toast } from "sonner";

export function useWhatsAppAdminDevices() {
  const { t } = useI18n();
  ...
  const fetchDevices = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await whatsappAdminApi.getAdminDevices({
        page,
        pageSize,
  ...
  const deleteDevice = async (id: string) => {
    try {
      await whatsappAdminApi.deleteAdminDevice(id);
...
export default useWhatsAppAdminDevices;
