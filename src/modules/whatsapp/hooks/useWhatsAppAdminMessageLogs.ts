...
import { AdminMessageLogItem } from "../types/admin.types";
import { whatsappAdminApi } from "../api/admin.api";
import { toast } from "sonner";

export function useWhatsAppAdminMessageLogs() {
  const { t } = useI18n();
  ...
  const fetchLogs = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await whatsappAdminApi.getAdminMessageLogs({
        page,
        pageSize,
  ...
  const deleteLog = async (id: string) => {
    try {
      await whatsappAdminApi.deleteAdminMessageLog(id);
...
export default useWhatsAppAdminMessageLogs;
