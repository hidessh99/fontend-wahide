...
import { useWhatsAppUserDevices } from "@/modules/whatsapp/hooks/useWhatsAppUserDevices";
import { Button } from "@/components/ui/button";
import { ErrorBoundary } from "@/components/layout/shared/ErrorBoundary";
import { useI18n } from "@/lib/i18n/context";
import { Smartphone, RefreshCw } from "lucide-react";

export function WhatsAppUserDevicesView() {
  const { t } = useI18n();
  const deviceState = useWhatsAppUserDevices();
        ...
        <div className="hidden sm:flex items-center gap-2">
          ...
          >
            <RefreshCw
              className={`size-3.5 ${deviceState.isLoading ? "animate-spin" : ""}`}
            />
            <span>{t("whatsapp.refreshListBtn")}</span>
          </Button>
        </div>
      </div>
...
export default WhatsAppUserDevicesView;
