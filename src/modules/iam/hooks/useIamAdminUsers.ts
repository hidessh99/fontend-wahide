...
export function useIamAdminUsers() {
  ...
  const fetchAdminData = useCallback(async () => {
    setIsLoading(true);
    try {
      const usrData = await iamAdminApi.getUsers({ page: 1, pageSize: 100 });
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        const usrData = await iamAdminApi.getUsers(
          { page: 1, pageSize: 100 },
          controller.signal,
        );
  ...
  const adjustBalance = async (payload: AdjustBalanceInput) => {
    try {
      await iamAdminApi.adjustUserBalance(payload);
      setUsers((prev) =>
        prev.map((u) => {
  ...
  const updateUser = async (userId: string, payload: UpdateUserInput) => {
    try {
      await iamAdminApi.updateUser(userId, payload);
      setUsers((prev) =>
        prev.map((u) => {
          if (u.id === userId) {
            return {
