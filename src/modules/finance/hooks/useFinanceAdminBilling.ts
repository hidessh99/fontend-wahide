...
export function useFinanceAdminBilling() {
  const { t } = useI18n();
  ...
  const fetchBillings = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await financeAdminApi.getAdminBillings({
        page,
        pageSize,
        search: searchQuery,
  ...
  const updateStatus = async (id: string, status: "EXPIRED" | "CANCELLED") => {
    try {
      await financeAdminApi.updateAdminBillingStatus(id, { status });
      setBillings((prev) =>
        prev.map((b) =>
  ...
  const deleteBilling = async (id: string) => {
    try {
      await financeAdminApi.deleteAdminBilling(id);
...
export default useFinanceAdminBilling;
