...
import { AdminSubscriptionItem } from "../types/admin.types";
import { subscriptionAdminApi } from "../api/admin.api";
import { toast } from "sonner";
import { useI18n } from "@/lib/i18n/context";

export function useSubscriptionAdminSubscriptions() {
  const { t } = useI18n();
  ...
  const fetchSubscriptions = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await subscriptionAdminApi.getAdminSubscriptions({
        page,
        pageSize,
  ...
  const expireSubscription = async (id: string) => {
    try {
      await subscriptionAdminApi.expireAdminSubscription(id);
...
export default useSubscriptionAdminSubscriptions;
