...
import { UserItem } from "@/modules/iam/types/admin.types";
import { iamAdminApi } from "@/modules/iam/api/admin.api";
  ...
  useEffect(() => {
    if (broadcastTarget === "SPECIFIC" && !hasLoadedUsersRef.current) {
      ...
      const loadUsers = async () => {
        ...
        try {
          const res = await iamAdminApi.getUsers({ page: 1, pageSize: 100 });
          setUsers(res.users);
        } catch {
          // Handled gracefully
        } finally {
