...
import { systemAdminApi } from "../api/admin.api";
import { toast } from "sonner";

export function useSystemAdminNotifications() {
  const { t } = useI18n();
  ...
  const fetchQueues = useCallback(async () => {
    setIsLoading(true);
    try {
      const res = await systemAdminApi.getAdminQueues({
        page,
        pageSize,
        search: searchQuery,
  ...
  const deleteQueue = async (id: string) => {
    try {
      await systemAdminApi.deleteAdminQueue(id);
  ...
  const sendBroadcastAll = async (subject: string, message: string) => {
    setIsSending(true);
    try {
      const res = await systemAdminApi.broadcastToAllUsers({
        subject,
        message,
      });
  ...
  const sendBroadcastSpecific = async (
    ...
    setIsSending(true);
    try {
      const res = await systemAdminApi.broadcastToSpecificUsers({
        userIds,
        subject,
        message,
      });
  ...
  const sendDirectEmail = async (
    ...
    setIsSending(true);
    try {
      const res = await systemAdminApi.createDirectEmailQueue({
        email,
        name,
        subject,
        message,
  ...
  const sendDirectEmailsBatch = async (
    ...
    try {
      ...
      for (const target of targets) {
        await systemAdminApi.createDirectEmailQueue({
          email: target.email,
